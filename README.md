...
**同步代碼**與**密碼**，到其他裝置輸入同一組代碼＋密碼「加入同步」，之後每次作答都會
自動、節流地把學習紀錄同步到其他已加入的裝置（存檔即上傳；接收端在畫面被實際碰一下、
或重新整理／切回分頁時檢查更新，同一台裝置的檢查最密集每 5 秒一次）。也可以隨時按
「立即同步」手動觸發一次。
...
裝置的總作答次數大於或等於伺服器，才會真的上傳。「清除全部學習紀錄」是唯一的例外——
清除後總作答次數會變成 0，一定會被判定為落後，所以會另外跳出確認，問是否要連同步的紀錄
也一起清除；選是才會真的覆蓋過去（此動作無法復原，其他已加入同步的裝置下次同步時也會
被清空），選否的話同步的紀錄不受影響，而且下次自動同步時，這台裝置本身也會被伺服器上的
舊紀錄補回來。
