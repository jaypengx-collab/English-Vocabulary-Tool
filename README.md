...
- **四種測驗模式**（首頁「測驗模式」區塊選擇）：
  - **⚖️ 自動平衡**（預設）：依你目前「答錯待複習／學習中／新字」各有多少單字，即時
    ...
    （15%～75% 之間），但永遠不會變成 100% 複習或 0% 新字，避免剛開始學、新字量本來就
    很多時被複習題目淹沒。複習量裡答錯的單字又比學習中的單字更優先。**這個比例在回合中
...
- **依反應時間排定複習優先度，且按單字長度公平比較**：系統會先用你所有單字的作答時間，
  ...
  短單字則永遠被誤判成「很熟」，跟實際記不記得住完全無關。「答錯待複習」「學習中」題目
  會用加權隨機的方式抽選，比這個「同長度預期時間」慢、比較久沒複習的單字被抽中的機率
  較高，但不是每次都固定抽同一個字——這次答得快，下次被抽中的機率就會降低，換其他字
  有機會出現。「複習」頁面依「反應時間」排序時用的也是同一套公平比較，不會變成單純把
  最長的單字排在最前面。
...
「複習」分頁把**答錯待複習**與**學習中**拆成兩個分頁籤，同一時間只看一種，畫面更清爽；
每種底下又有兩種瀏覽方式可以切換：

- **📋 列表**（預設）：原本就有的可搜尋、可排序、分頁瀏覽的單字卡片清單——答錯待複習會
  顯示正確拼法與打錯的字母對照，學習中會顯示目前的連續正確次數，想要快速掃過或用關鍵字
  搜尋時用這個。
- **🎴 卡片瀏覽**：新增的大卡片模式，一次只顯示一個單字，字級放大、按鈕變大，適合專心
  一個一個背。點一下卡片播放發音並翻面看中文意思（答錯待複習的卡片翻面後也會顯示打錯
  對照）；用手指左右滑動（或按 ‹ › 按鈕）換到上一個／下一個單字，也會提前載入下一張
  卡片的發音，減少切換等待。

卡片瀏覽模式下有一個「📝 測驗這些單字」按鈕，會直接拿**目前畫面上這個分頁／這個搜尋
篩選後的單字組合**開一場真正的聽寫測驗（沿用跟首頁測驗完全相同的引擎與畫面），**答對
答錯一樣會正常計入學習紀錄、影響單字狀態**——複習卡片本身只是瀏覽、不算作答，但接著
用這個按鈕考自己，答對就會讓連續正確次數往上累加、真的有機會讓一個字從「學習中」變成
「已熟記」；這正是複習功能存在的意義，如果考試結果不算數，複習就沒有實際用處了。
...
- `app.js`：畫面渲染、事件處理、`localStorage` 讀寫、語音播放（含 Web Audio API 的
  `AudioContext` 管理與下一題音檔的提前載入，見下方〈單字發音〉）、四種測驗模式與
  自動平衡模式的即時回合中重算（`rebalanceAutoModeTail`）、複習分頁的列表／卡片瀏覽
  雙視圖與滑動手勢（Pointer Events）、`showConfirmDialog` 自訂確認彈窗（取代
...
聽目前這一題（或複習卡片瀏覽模式的這一張卡片）的同時，系統已經在背景把下一題／下一張
