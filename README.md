...
或重新整理／切回分頁也會檢查更新（同一台裝置的檢查最密集每 5 秒一次）；按「開始單字
測驗」／「開始複習測驗」的當下也會先花最多幾秒鐘抓一次最新進度，再照抓回來的紀錄出題——
避免這台裝置擺著沒動、其他裝置同時練了一段時間之後，這一回合是根據舊資料出的（抓不到時
不會卡住，照本機現有的資料照常開始）。也可以隨時按「立即同步」手動觸發一次。
