...
- **單字測驗**：只有一種模式（原本的「單字測驗」「複習測驗」已合併），播放預先錄製的
  高品質單字發音（見下方「單字發音」），聽完後輸入拼字，立即核對答案並顯示中文意思。
  **照時間長度出題，不是照題數**：首頁用滑桿拖曳（或點選快速選項）決定要練習幾分鐘
  （預設 10 分鐘，2～30 分鐘可調），時間到會自動結束並顯示成績；實際會出幾題完全依你
  自己的作答速度而定，不用自己抓題數。這回合「新字／答錯待複習／學習中」單字各佔多少
  比例，也用另外三個滑桿自由調整（拖一個，另外兩個會自動增減，三者總和固定 100%）——
  原本的「單字測驗」（80/10/10）、「複習測驗」（0/70/30，新字比例設為 0%）都只是這組
  比例滑桿上的兩個常用組合，首頁各留了一個快速按鈕。若選擇的範圍字數不足，系統會自動
  用其他非 0% 的類別補足，但 0% 的類別永遠不會出現；同一回合不會出現重複單字，已熟記
  的單字不會再出現。選擇的等級＋比例組合下如果完全沒有符合條件的單字，會顯示提示畫面
  而不是直接出錯，可以回首頁調整。測驗畫面計時器旁有一個「結束」按鈕，可以隨時提早結束
  這一回合並直接看目前的成績，不用等時間到或先跳去別的分頁再確認離開。
...
- **依反應時間排定複習優先度**：系統會先算出你目前所有單字的平均反應時間（你自己的
  打字速度基準），「答錯待複習」「學習中」題目會用加權隨機的方式抽選，比這個平均速度
  慢、比較久沒複習的單字被抽中的機率較高，但不是每次都固定抽同一個字——這次答得快，
  下次被抽中的機率就會降低，換其他字有機會出現。
...
或重新整理／切回分頁也會檢查更新（同一台裝置的檢查最密集每 5 秒一次）；按「開始測驗」
的當下，題目一樣會照本機現有的資料立刻出現（手機鍵盤、發音都不會因為等網路而延遲），
同時在背景抓一次最新進度——真的抓到其他裝置的更新時，只有「這回合還沒出到」的題目會
換成新資料，已經看到／答過的題目完全不受影響。也可以隨時按「立即同步」手動觸發一次。
...
- `logic.js`：不依賴 DOM、可獨立單元測試的核心邏輯——單字狀態判斷、依反應時間排序的
  複習優先度、`selectQuestions` 的比例式選題（新字／答錯待複習／學習中，任意比例組合，
  取代原本兩個模式各自的 80/10/10、70/30 固定邏輯）、進度統計、資料格式轉換
  （migration）。
- `app.js`：畫面渲染、事件處理、`localStorage` 讀寫、語音播放（含 Web Audio API 的
  `AudioContext` 管理，見下方〈單字發音〉）、`showConfirmDialog` 自訂確認彈窗（取代
  `window.confirm()`），呼叫 `logic.js` 提供的函式；也透過 `window.VocabState` 把
  `progressStore`／`settings` 開放給 `sync.js` 讀寫，透過 `window.VocabUI` 把確認彈窗
  開放給 `sync.js` 使用。
...
- `tests/logic.test.js`：涵蓋作答紀錄、狀態判斷（含連續答對重置）、時間優先度加權、選題
  比例與備援（含只挑單一類別、某類別比例為 0% 時絕不當備援）、打錯答案記錄、資料轉換
  等情境，執行方式：`npm test`（需要 Node.js）。
...
不只會被「暫停」，有時會被系統整個「關閉」以釋放音訊資源——被關閉後就永遠無法再
`resume()`，回到頁面後聲音會完全啞掉，直到整個網頁重新整理才會恢復。`app.js` 的
`getAudioContext()` 會偵測這個狀態，自動建立一個新的 `AudioContext` 取代已關閉的那個，
所以回到頁面後按播放/重播就能正常出聲，不需要重新整理。
